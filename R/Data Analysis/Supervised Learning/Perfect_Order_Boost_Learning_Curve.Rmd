---
title: "Boosting Learning Curve Analysis of Perfect Order Data"
...
modelTune = readRDS("Perfect_Order_Boost_modelTune.rds")
...
n.trees = modelTune$bestTune$n.trees
interaction.depth = modelTune$bestTune$interaction.depth
shrinkage = modelTune$bestTune$shrinkage

cvCtrl = trainControl(method = "repeatedcv", repeats = 3)
search.grid <- expand.grid(.n.trees = n.trees, 
...
LCSets = function(x){
  ...
  modelLC = train(Outcome ~ ., data = trainLC, method = "gbm", 
                  tuneGrid = search.grid, verbose = F, trControl = cvCtrl)
...
saveRDS(results, "PO_Boost_LC_results.rds")
...
ggsave("PO_Boost_LC.png", width = 4, height = 4, dpi = 120)
dev.off()
```
...
trainingTime = microbenchmark(
    train(Outcome ~ ., data = training, method = "gbm", 
                    tuneGrid = search.grid, verbose = F, trControl = cvCtrl), times = 10L)
